...
import { ErrorsParsingService, NotificationService } from '../../services';

import { Log, Level } from 'ng2-logger/ng2-logger';
const log = Log.create('TasksListCmp');
log.color = 'orange';
...
export class TasksListCmp {
  ...
  constructor(
    ...
    private EPS: ErrorsParsingService,
  ...
  getTasks() {
    ...
    this.tasksListApi.getTasks().subscribe((tasks: Task[]) => {
      ...
      log.d('getTasks', error);
      // const errorName = this.EPS.parse(error);
      this.notifySrv.show(error.title);
    });
  }
}